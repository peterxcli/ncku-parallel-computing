...
void distributeData(long long n, int rank, int world_size, int **loc_w, int *loc_begin, int *loc_end) {
    long long displs = n;
    for (int i = 0; i < world_size; ++i) {
        int sendcount = ((n - 1) / world_size + (i < (n - 1) % world_size)) * n;
        if (rank == 0 && i != 0) {
            MPI_Send(w + displs, sendcount, MPI_INT, i, TAG_DATA, MPI_COMM_WORLD);
        }

        if (rank == i) {
            *loc_w = (int *)malloc(sendcount * sizeof(int));
            if (!*loc_w) {
            ...
            *loc_end = *loc_begin + sendcount / n;
            ...
            if (i != 0) {
                MPI_Recv(*loc_w, sendcount, MPI_INT, 0, TAG_DATA, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            } else {
                memcpy(*loc_w, w + displs, sendcount * sizeof(int));
            }
        }
        displs += sendcount;
    }
...
void initializeDistances(long long n, int loc_n, int *loc_w, int *dist, bool *vis) {
...
void dijkstra(long long n, int rank, int loc_begin, int loc_end, int *loc_w, int *dist, bool *vis) {
